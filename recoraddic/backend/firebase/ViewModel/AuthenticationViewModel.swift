...
//  testingSpace_firebase
//
//  Created by 김지호 on 12/5/24.
//
...
class AuthenticationViewModel: ObservableObject {
    ...
    func deleteAccount() async -> Bool {
        ...
        let signedInWithGoogle = user.providerData.contains { $0.providerID == "google.com" }
        
        
        do {
            
            if signedInWithGoogle {
                let signInWithGoogle = await self.signInWithGoogle()
                if !signInWithGoogle { return false }
            }
            else if needsReauth || needsTokenRevocation {
...
extension AuthenticationViewModel {
    func signInWithGoogle() async -> Bool {
        // This code needs to be implemented, unless you didn't add GIDClientID in info.plist
